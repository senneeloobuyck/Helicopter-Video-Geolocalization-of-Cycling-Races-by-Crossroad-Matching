...
def perspective_transform(frames_from_vid_dir) :
    for img_file in os.listdir(frames_from_vid_dir) :
        img = cv2.imread(os.path.join(frames_from_vid_dir, img_file))
        ...
        offset = 50
        tl = [offset, offset]
        tr = [640-offset, 2*offset]
        bl = [2*offset, 480-offset]
        br = [640-offset, 480-offset]
        ...
        # Apply the perspective transformation to obtain the bird's-eye view
        birdseye_view = cv2.warpPerspective(img, M, (640, 480), cv2.INTER_LINEAR)

        # Display the original image and the bird's-eye view
